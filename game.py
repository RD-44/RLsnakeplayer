...
class SnakeGameAI:
    ...
    def play_step(self, action):
        ...
        bad = False
        if self.head in self.previousheads:
            self.previousheads[self.head] += 1
        else:
            self.previousheads[self.head] = 1
        if self.previousheads[self.head] > 3:
            bad = True
        ...
        if self.head == self.food:
            self.score += 1
            self.previousheads.clear()
            reward = 10 # reward ai for eating food
            #self.frame_iteration = 0
            self._place_food()
        else:
            self.snake.pop()
